...
$users_id = $_GET['id'];
$query = "SELECT * from po_transactions WHERE id='$users_id'";
$result = mysqli_query($link, $query) or die(mysqli_error($link));
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)){

        $po_trans_id;
        $po_qty$_POST   = $row['po_qty'];
        $po_unit$_POST  = $row['po_unit'];
        $po_description$_POST = $row['po_description'];
        $po_unit_price$_POST  = $row['po_unit_price'];
        $po_total_amount$_POST  = $row['po_total_amount'];
        $user;
